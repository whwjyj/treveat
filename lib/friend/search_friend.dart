...
Future<List<model_friend>> fetchFriend(String name) async {
  final response = await http.get(Uri.parse(
      'http://dongseok1.dothome.co.kr/treveat/mUsers/selectUsersByName.php?start=0&end=10&name=$name'));
...
class search_friend extends StatefulWidget {
  ...
  search_friend(this.name, {Key? key}) : super(key: key);

  @override
  State<search_friend> createState() => _search_friend();
}

class _search_friend extends State<search_friend> {
  late Future<List<model_friend>> futurefriend;

  @override
  void initState() {
    super.initState();
    futurefriend = fetchFriend(widget.name);
  }
